...
float **create_region_centers(uint16_t nr_regions) {
	if (world->grid == NULL || nr_regions == 0) return NULL;
	int i, j, tmp;
	int numbers[world->grid->height * world->grid->width];
	for (i = 0; i < world->grid->height * world->grid->width; i++) numbers[i] = i;
	for (i = 0; i < world->grid->height * world->grid->width / 4; i++) {
		j = i + rand() / (RAND_MAX / (world->grid->height * world->grid->width - i) + 1); 
		tmp = numbers[j];
		numbers[j] = numbers[i];
		numbers[i] = tmp;
	}   
...
void create_regions(uint16_t nr_regions) {
	if (world->regionlist != NULL || nr_regions == 0) return;
	int i;
...
void assign_tiles_to_centers2() {
	...
	if (world->grid == NULL || nr_regions == 0 || region_centers == NULL) return;
	int i, j, k;
	...
	for (i = 0; i < world->grid->height; i++) {
		for (j = 0; j < world->grid->width; j++) {
			closest_center = 0;
			min_distance = (float) sqrt(powf(((float) i + 0.5 - region_centers[closest_center][0]), 2) + powf(((float) j + 0.5 - region_centers[closest_center][1]), 2));
			for (k = 1; k < nr_regions; k++) {
				tmp = (float) sqrt(powf(((float) i + 0.5 - region_centers[k][0]), 2) + powf(((float) j + 0.5 - region_centers[k][1]), 2));
				if (tmp < min_distance) {
					closest_center = k;
					min_distance = tmp;
				}
...
void assign_tiles_to_centers() {
	...
	for (i = 0; i < world->grid->height; i++) {
		for (j = 0; j < world->grid->width; j++) {
			...
			while (current != NULL) {
//			for (k = 1; k < nr_regions; k++) {
				tmp = (float) sqrt(powf(((float) i + 0.5 - region_centers[counter][0]), 2) + powf(((float) j + 0.5 - region_centers[counter][1]), 2));
				if (tmp < min_distance) {
...
void recalculate_region_centers2() {
	...
	for (k = 0; k < nr_regions; k++) {
		...
		for (i = 0; i < world->grid->height; i++) {
			for (j = 0; j < world->grid->width; j++) {
				if (world->grid->tiles[i][j]->region->id == k + 1) {
					cumul_h += i;
					cumul_w += j;
				}
			}
...
//	while (counter < nr_regions) {
		...
		for (i = 0; i < world->grid->height; i++) {
			for (j = 0; j < world->grid->width; j++) {
				if (world->grid->tiles[i][j]->region->id == current->id) {
					cumul_h += i;
					cumul_w += j;
				}
			}
...
void voronoi() {
	...
	int i;
	uint16_t nr_regions = (world->grid->height) * (world->grid->width) / region_size;
	...
	for (i = 0; i < nr_regions; i++) free(region_centers[i]);
	free(region_centers);
}
