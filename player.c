...
void remove_player(player_t * player)
{
	...
	while (current_region != NULL) {
		if (current_region->owner != NULL
		    && current_region->owner->id == player->id) {
	...
	while (dipstatus != NULL) {
		if (dipstatus->player1 == player
		    || dipstatus->player2 == player) {
	...
	player_t *current = world->playerlist;
	while (current != NULL) {
		if (current->heir != NULL && current->heir->id == player->id)
			current->heir = NULL;
		if (current->lord != NULL && current->lord->id == player->id)
			current->lord = NULL;
		current = current->next;
	}
...
{
	while (world->playerlist != NULL)
		remove_player(world->playerlist);
...
**/
	...
	unsigned int months =
	    (player->birthdate.tm_year + MIN_AGE) * 12 +
	    player->birthdate.tm_mon;
...
{
	if (player == NULL
	    || (successor != NULL && player->id == successor->id))
	...
	**/
	...
	while (current_player != NULL) {
		...
		/* if a player reaches old age, he loses the game */
			 ...
			 current_player->deathdate.tm_year +
			 12 * current_player->deathdate.tm_mon) {
...
void succession(player_t * player)
{
	...
	if (player->heir != NULL) {
		...
		add_to_cronicle
		    ("%s %s had not named a heir, and their lord, %s %s, inherited everything.\n",
		     ranklist[player->rank], player->name, ranklist[heir->rank],
		     heir->name);
	} else {
	...
	piece_t *current_piece = world->piecelist;
	while (current_piece != NULL) {
		if (current_piece->owner->id == player->id
		...
		if (player->lord == NULL)
			...
			add_to_cronicle
			    ("%s %s paid homage to %s %s for their title.\n",
			     ranklist[heir->rank], heir->name,
			     ranklist[player->lord->rank], player->lord->name);
