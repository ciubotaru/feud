...
dipstatus_t *set_diplomacy(player_t * player1, player_t * player2,
			   const unsigned int status)
{
	...
	if ((player1->lord != NULL && player1->lord->id == player2->id)
	    || (player2->lord != NULL && player2->lord->id == player1->id))
		if (status != ALLIANCE)
			return NULL;
...
//printf("Diplomacy list does not exist. Creating...\n");
	...
	while (current != NULL) {
		if ((current->player1 == player1 && current->player2 == player2)
		    || (current->player1 == player2
			&& current->player2 == player1)) {
...
dipstatus_t *get_dipstatus(player_t * player1, player_t * player2)
{
	dipstatus_t *current = world->diplomacylist;
	while (current != NULL) {
		if ((current->player1->id == player1->id
		     && current->player2->id == player2->id)
		    || (current->player1->id == player2->id
			&& current->player2->id == player1->id)) {
			return current;
		}
		if (current->next == NULL)
			break;
	...
	**/
	if (player_rank >= lord_rank) {
		...
		while (current_player != NULL) {
			/* free vassals */
			if (current_player->lord != NULL
			    && current_player->lord->id == player->id
			    && get_player_rank(current_player) >= player_rank)
				current_player->lord = NULL;
...
void promote_soldier(player_t * player, piece_t * piece, region_t * region,
		     char *name)
{
	if (player == NULL || piece == NULL || region == NULL)
		return;
	/* promotion costs money */
	if (player->money < COST_SOLDIER)
		return;
	...
	if (count_regions_by_owner(player) < 2)
		return;
...
uint16_t count_vassals(player_t * player)
{
	...
	while (current_vassal != NULL) {
		if (current_vassal->lord != NULL
		    && current_vassal->lord->id == player->id)
			counter++;
