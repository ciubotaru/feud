...
int add_to_cronicle(char *format, ...) {
	...
	char date_char[100];
	sprintf(date_char, "%s of year %d: ", months[world->current_time.tm_mon], world->current_time.tm_year);
...
int deserialize_game_metadata(char **buffer, int *pos) {
	...
	memcpy(&next_player_id_be, *buffer + sizeof(uint16_t) + 1, sizeof(uint16_t));
	world->next_player_id = be16toh(next_player_id_be);
	uint16_t next_piece_id_be = 0;	
	memcpy(&next_piece_id_be, *buffer + 1 + 2 * sizeof(uint16_t), sizeof(uint16_t));
	world->next_piece_id = be16toh(next_piece_id_be);
	uint16_t selected_player_be = 0;
	memcpy(&selected_player_be, *buffer + 1 + 3 * sizeof(uint16_t), sizeof(uint16_t));
	world->selected_player = be16toh(selected_player_be);
	memcpy(&moves_left, *buffer + 1 + 4 * sizeof(uint16_t), sizeof(unsigned char));
...
**/
	...
	for (i = 0; i < nr_players; i++) {
		...
		memcpy(&money_be, *buffer + buffer_pos + offset, sizeof(uint16_t));
		offset += sizeof(uint16_t);
		set_money(current, be16toh(money_be));
		memcpy(&(current->rank), *buffer + buffer_pos + offset, sizeof(unsigned char));
		offset += sizeof(unsigned char);
		memcpy(&birthdate_year_be, *buffer + buffer_pos + offset, sizeof(uint16_t));
		offset += sizeof(uint16_t);
		current->birthdate.tm_year = be16toh(birthdate_year_be);
		memcpy(&(current->birthdate.tm_mon), *buffer + buffer_pos + offset, 1);
		offset += 1;
		memcpy(&deathdate_year_be, *buffer + buffer_pos + offset, sizeof(uint16_t));
		offset += sizeof(uint16_t);
		current->deathdate.tm_year = be16toh(deathdate_year_be);
		memcpy(&(current->deathdate.tm_mon), *buffer + buffer_pos + offset, 1);
...
//		memcpy(&size_be, *buffer + buffer_pos + sizeof(uint16_t), sizeof(uint16_t));
		...
		memcpy(&owner_be, *buffer + buffer_pos + sizeof(uint16_t) + 17, sizeof(uint16_t));
		current = add_region(name);
		current->id = be16toh(id_be);
		current->size = 0;//be16toh(size_be);
//		current->owner = get_player_by_id(be16toh(owner_be));
		change_region_owner(get_player_by_id(be16toh(owner_be)), current);
...
int deserialize_grid(char **buffer, int *pos) {
	...
	memcpy(&width_be, *buffer + buffer_pos + sizeof(uint16_t), sizeof(uint16_t));
	if (world->grid != NULL) remove_grid();
	world->grid = create_grid(be16toh(height_be), be16toh(width_be));
	if (!world->grid) return 1;
	...
	for (i = 0; i < world->grid->height; i++) {
		for (j = 0; j < world->grid->width; j++) {
			memcpy(&world->grid->tiles[i][j]->walkable, *buffer + buffer_pos, 1);
			memcpy(&region_be, *buffer + buffer_pos + 1, sizeof(uint16_t)); /* id */
			change_tile_region(get_region_by_id(be16toh(region_be)), world->grid->tiles[i][j]);
...
int deserialize_pieces(char **buffer, int *pos) {
	...
	for (i = 0; i < nr_pieces; i++) {
		...
		memcpy(&height_be, *buffer + buffer_pos + 1 + sizeof(uint16_t), sizeof(uint16_t));
		memcpy(&width_be, *buffer + buffer_pos + 1 + 2 * sizeof(uint16_t), sizeof(uint16_t));
		memcpy(&owner_be, *buffer + buffer_pos + 1 + 3 * sizeof(uint16_t), sizeof(uint16_t));
		current = add_piece(type, be16toh(height_be), be16toh(width_be), get_player_by_id(be16toh(owner_be)));
		if (!current) return 1;
		current->id = be16toh(id_be);
		world->next_piece_id = current->id;
		world->grid->tiles[be16toh(height_be)][be16toh(width_be)]->piece = current;
...
int deserialize_diplomacy(char **buffer, int *pos) {
	...
	for (i = 0; i < nr_dipstat; i++) {
		...
		memcpy(&status, *buffer + buffer_pos + sizeof(uint16_t) * 2, sizeof(unsigned char));
...
int deserialize_dipoffer(char **buffer, int *pos) {
	...
	for (i = 0; i < nr_dipoffer; i++) {
		...
		memcpy(&offer, *buffer + buffer_pos + sizeof(uint16_t) * 2, sizeof(unsigned char));
...
int serialize_game_metadata(char **buffer) {
	...
	memcpy(*buffer + sizeof(uint16_t), &(world->current_time.tm_mon), sizeof(unsigned char));
	memcpy(*buffer + sizeof(uint16_t) + sizeof(unsigned char), &next_player_id_be, sizeof(uint16_t));
	memcpy(*buffer + sizeof(unsigned char) + 2 * sizeof(uint16_t), &next_piece_id_be, sizeof(uint16_t));
	memcpy(*buffer + sizeof(unsigned char) + 3 * sizeof(uint16_t), &selected_player_be, sizeof(uint16_t));
	memcpy(*buffer + sizeof(unsigned char) + 4 * sizeof(uint16_t), &(world->moves_left), sizeof(unsigned char));
...
int serialize_playerlist(char **buffer) {
	...
	int total_len = PLAYERLIST_METADATA_SIZE + PLAYERLIST_UNIT_SIZE * nr_players;
...
int serialize_regionlist(char **buffer) {
	...
	int total_len = REGIONLIST_METADATA_SIZE + REGIONLIST_UNIT_SIZE * nr_regions;
	/* id, name, owner */
...
**/
		memcpy(*buffer + pos + sizeof(uint16_t), current->name, strlen(current->name));
		owner_be = (current->owner != NULL ? htobe16(current->owner->id) : 0);
		memcpy(*buffer + pos + sizeof(uint16_t) + 17, &owner_be, sizeof(uint16_t));
...
int serialize_grid(char **buffer) {
	...
	int total_len = GRID_METADATA_SIZE + GRID_UNIT_SIZE * world->grid->height * world->grid->width;
	...
	for (i = 0; i < world->grid->height; i++) {
		for (j = 0; j < world->grid->width; j++) {
			memcpy(*buffer + pos, & world->grid->tiles[i][j]->walkable, 1);
			region_be = (world->grid->tiles[i][j]->region != NULL ? htobe16(world->grid->tiles[i][j]->region->id) : 0);
	...
	for (i = 0; i < world->grid->height; i++) {
		for (j = 0; j < world->grid->width; j++) {
			if (world->grid->tiles[i][j]->piece != NULL) {
				id_be = htobe16(world->grid->tiles[i][j]->piece->id);
				memcpy(*buffer + pos, & id_be, sizeof(uint16_t));
				memcpy(*buffer + pos + sizeof(uint16_t), &(world->grid->tiles[i][j]->piece->type), 1);
				height_be = htobe16(i);
				memcpy(*buffer + pos + sizeof(uint16_t) + 1, &height_be, sizeof(uint16_t));
				width_be = htobe16(j);
				memcpy(*buffer + pos + sizeof(uint16_t) + 1 + sizeof(uint16_t), &width_be, sizeof(uint16_t));
				owner_be = htobe16(world->grid->tiles[i][j]->piece->owner->id);
				memcpy(*buffer + pos + sizeof(uint16_t) + 1 + 2 * sizeof(uint16_t), &owner_be, sizeof(uint16_t));
...
int serialize_feudal(char **buffer, uint16_t nr_feudal) {
	...
	while (current != NULL) {
		if (current->lord != NULL) {
			...
			memcpy(*buffer + pos + sizeof(uint16_t), &lord_id_be, sizeof(uint16_t));
...
int serialize_heir(char **buffer, uint16_t nr_heir) {
	...
	while (current != NULL) {
		if (current->heir != NULL) {
			...
			memcpy(*buffer + pos + sizeof(uint16_t), &heir_id_be, sizeof(uint16_t));
			pos += HEIR_UNIT_SIZE;
		}
		current = current->next;
	}
...
int serialize_diplomacy(char **buffer, uint16_t nr_dipstat) {
	...
	int total_len = DIPLOMACY_METADATA_SIZE + DIPLOMACY_UNIT_SIZE * nr_dipstat;
	...
	while (current != NULL) {
		...
		memcpy(*buffer + pos + sizeof(uint16_t), &player2_id_be, sizeof(uint16_t));
		memcpy(*buffer + pos + sizeof(uint16_t) * 2, &(current->status), sizeof(unsigned char));
...
int serialize_dipoffer(char **buffer, uint16_t nr_dipoffer) {
	...
	int total_len = DIPOFFER_METADATA_SIZE + DIPOFFER_UNIT_SIZE * nr_dipoffer;
	...
	while (current != NULL) {
		if (current->pending_offer != NULL) {
			...
			memcpy(*buffer + pos + sizeof(uint16_t), &to_id_be, sizeof(uint16_t));
			memcpy(*buffer + pos + sizeof(uint16_t) * 2, &(current->pending_offer->offer), sizeof(unsigned char));
...
unsigned int save_game() {
	...
	char *playerlist_buffer = calloc(1, PLAYERLIST_METADATA_SIZE + count_players() * PLAYERLIST_UNIT_SIZE + 1);
	...
	char *regionlist_buffer = calloc(1, REGIONLIST_METADATA_SIZE + count_regions() * REGIONLIST_UNIT_SIZE + 1);
	...
	char *grid_buffer = calloc(1, GRID_METADATA_SIZE + world->grid->height * world->grid->width * GRID_UNIT_SIZE + 1);
	if (!grid_buffer) {
		fclose(fp);
		retval = 5;
		goto ret;
	...
	char *pieces_buffer = calloc(1, PIECES_METADATA_SIZE + nr_pieces * PIECES_UNIT_SIZE + 1);
	if (!pieces_buffer) {
		fclose(fp);
		retval = 8;
		goto ret;
	...
	player_t *current = world->playerlist;
	while (current != NULL) {
		if (current->lord != NULL) nr_feudal++;
		current = current->next;
	}
	char *feudal_buffer = calloc(1, FEUDAL_METADATA_SIZE + nr_feudal * FEUDAL_UNIT_SIZE + 1);
	if (!feudal_buffer) {
		fclose(fp);
		retval = 11;
		goto ret;
	...
	while (current != NULL) {
		if (current->heir != NULL) nr_heir++;
		current = current->next;
	}
	char *heir_buffer = calloc(1, HEIR_METADATA_SIZE + nr_heir * HEIR_UNIT_SIZE + 1);
	if (!heir_buffer) {
		fclose(fp);
		retval = 14;
		goto ret;
	...
	char *diplomacy_buffer = calloc(1, DIPLOMACY_METADATA_SIZE + nr_dipstat * DIPLOMACY_UNIT_SIZE + 1);
	if (!diplomacy_buffer) {
		fclose(fp);
		retval = 17;
		goto ret;
	...
	while (current_diplomacy != NULL) {
		if (current_diplomacy->pending_offer != NULL) nr_dipoffer++;
		current_diplomacy = current_diplomacy->next;
	}
	char *dipoffer_buffer = calloc(1, DIPOFFER_METADATA_SIZE + nr_dipoffer * DIPOFFER_UNIT_SIZE + 1);
	if (!dipoffer_buffer) {
		fclose(fp);
		retval = 20;
		goto ret;
