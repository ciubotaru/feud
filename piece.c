...
piece_t *create_piecelist() {
	/* single instance */
	if (world->piecelist != NULL) return world->piecelist;
	world->piecelist = calloc(sizeof(piece_t), 1);
	if (world->piecelist == NULL) return NULL;
	return world->piecelist;
}

static void fill_piece_details(piece_t *piece, const unsigned char type, const uint16_t height, const uint16_t width, player_t *owner) {
...
piece_t *add_piece(const unsigned char type, const uint16_t height, const uint16_t width, player_t *owner) {
	if (owner == NULL) return NULL; /* bad owner */
	if (world->grid == NULL) return NULL; /* grid not created */
	if (world->grid->tiles[height][width]->walkable != 1) return NULL; /* unwalkable tile */
	if (world->grid->tiles[height][width]->piece != NULL) return NULL; /* tile occupied */
...
piece_t *get_noble_by_owner(player_t *owner) {
	...
	piece_t *current = world->piecelist;
	while (current != NULL) {
		if (current->owner->id == owner->id && current->type == 0) return current;
...
//			increment_gametime(); /* cycling around pieces of same player should not change time */
		...
		if (current->owner->id == active_player->id) return current;
	}
	return NULL;
}
...
void remove_piece(piece_t *piece) {
	...
	if (world->grid == NULL || piece == NULL || world->piecelist == NULL) return;
		...
		if (current == piece) {
			...
			if (previous == NULL) {
				...
				if (current->next == NULL) {
					world->grid->tiles[current->height][current->width]->piece = NULL;
					free(current);
				...
				/** first, but not last **/
					...
					world->grid->tiles[world->piecelist->height][world->piecelist->width]->piece = NULL;
...
void clear_piece_list() {
	...
	while (world->piecelist != NULL) remove_piece(world->piecelist);
...
uint16_t count_pieces_by_owner(player_t *owner) {
	...
	piece_t *current = world->piecelist;
	while (current != NULL) {
		if (current->owner->id == owner->id && current->type == 1) count++;
		current = current->next;
	}
	return count;
}
...
void update_army_ranking() {
	...
	while (player != NULL) {
		...
		while (player2->id != player->id) {
			if (count_pieces_by_owner(player) <= count_pieces_by_owner(player2)) player->rank_army++;
			else player2->rank_army++;
