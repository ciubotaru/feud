...
void map_editor() {
	...
	if (world->selected_region == 0 && world->regionlist != NULL) world->selected_region = world->regionlist->id;
...
/* ToDo change 2 to SHIP when we have ships */
	...
	/* player info */
	if (player != NULL) mvwprintw(local_win, 4, 50, "Sel. pl.: %s (%s)", player->name, ranklist[player->rank]);
	if (region != NULL) mvwprintw(local_win, 6, 50, "Sel. reg.: %s (%d)", region->name, region->size);
	else mvwprintw(local_win, 6, 50, "Sel. reg.: not selected");
/**
...
**/
	...
	switch (ch) {
		...
		case 'c':
			...
			if (tile->region == NULL) change_tile_region(region, world->grid->tiles[world->grid->cursor_height][world->grid->cursor_width]);
			else change_tile_region(NULL, world->grid->tiles[world->grid->cursor_height][world->grid->cursor_width]);
			break;
		case 'n':
			/* if tile not wlakable, just ignore */
			if (tile->walkable == 0) break;
			/* if tile is occupied, remove piece */
			if (piece != NULL) remove_piece(piece);
			/* if tile free, add soldier */
			else add_piece(NOBLE, world->grid->cursor_height, world->grid->cursor_width, player);
			break;
		case 'p':
		...
		case 'u':
			...
			if (piece != NULL) remove_piece(piece);
			/* if tile free, add soldier */
			else add_piece(SOLDIER, world->grid->cursor_height, world->grid->cursor_width, player);
			break;
		case 'v':
			current_screen = VALIDATE_DIALOG;
			break;
		case 'w':
			toggle_walkable(world->grid->cursor_height, world->grid->cursor_width);
			break;
		case '\t':
			if (piece != NULL) {
		...
		case '>':
			if (region != NULL) {
				if (region->next != NULL) region = region->next;
				else region = world->regionlist;
...
void new_game_dialog() {
	...
	for (i = 0; i < strlen(h_ch); i++) {
		if (!isdigit(h_ch[i])) break;
	}
	if (strlen(h_ch) > 0) h = atoi(h_ch);
	if (h == 0) h = 24;
	else if (h < 10) h = 10;
	else if (h > 100) h = 100;
	...
	for (i = 0; i < strlen(w_ch); i++) {
		if (!isdigit(w_ch[i])) break;
	}
	if (strlen(w_ch) > 0) w = atoi(w_ch);
	if (w == 0) w = 80;
	else if (w < 10) w = 10;
	else if (w > 100) w = 100;
	...
	for (i = 0; i < strlen(r_ch); i++) {
		if (!isdigit(r_ch[i])) break;
	}
	if (strlen(r_ch) > 0) region_size = atoi(r_ch);
	else if (region_size < 10) region_size = 10;
	else if (region_size > 100) region_size = 100;
	...
	for (i = 0; i < strlen(y_ch); i++) {
		if (!isdigit(y_ch[i])) break;
	}
	if (strlen(y_ch) > 0) y = atoi(y_ch);
	...
	for (i = 0; i < strlen(m_ch); i++) {
		if (!isdigit(m_ch[i])) break;
	}
	if (strlen(m_ch) > 0) m = atoi(m_ch);
	if (m > 12) m = 12;
	if (m > 0) m--;
		...
		case 'd':
			...
			if (current != NULL) {
				...
				if (playerlist_selector > 0) {
					playerlist_selector--;
					world->selected_player = get_player_by_order(playerlist_selector)->id;
				}
			}
			break;
		case 'e':
...
void add_player_dialog() {
	...
	while(1) {
		switch (stage) {
			case 0:
				if (error == 1) mvwprintw(local_win, 2, 2, "Name too short or not unique. Type another one:\n\n  ");
				else mvwprintw(local_win, 2, 2, "Type a unique name for the new player:         \n\n  ");
			...
			case 1:
				curs_set(FALSE);
				noecho();
				if (error == 1) mvwprintw(local_win, 6, 2, "Error. Choose a valid rank (1-4):       \n\n  ");
				else mvwprintw(local_win, 6, 2, "Choose player ranking (1-5 for knight, baron, count, duke or king), default is king:\n\n  ");
				rank = wgetch(local_win);
				if (rank == '1' || rank == '2' || rank == '3' || rank == '4' || rank == '5') {
					rank = (int) rank - '0';
					rank -= 1; /* to start notation from 0 */
				}
				else if (rank == 10) rank = KING;
				else error = 1;
				if (!error) {
					mvwprintw(local_win, 8, 2, "%s", ranklist[rank]);
					stage = 2;
				}
				break;
			case 2:
				curs_set(TRUE);
				echo();
				if (error == 1) mvwprintw(local_win, 10, 2, "Error. Try another number (0-999):       \n\n  ");
				else mvwprintw(local_win, 10, 2, "Type the initial amount of money (0-999):\n\n  ");
				money_ch[0] = '\0';
...
void regions_dialog() {
	...
	int regionlist_selector = get_region_order(get_region_by_id(world->selected_region));
	while (current != NULL) {
	...
	switch (user_move) {
		case 1065:
			if (regionlist_selector > 0) {
				--regionlist_selector;
				world->selected_region = get_region_by_order(regionlist_selector)->id;
			}
			break;
		case 1066:
...
void add_region_dialog() {
	...
	while(1) {
		switch (stage) {
			case 0:
				if (error == 1) mvwprintw(local_win, 2, 2, "Name too short or not unique. Type another one:\n\n  ");
				else mvwprintw(local_win, 2, 2, "Type a unique name for the new region (press Enter to return):\n\n  ");
				char name[17] = {0};
				wgetnstr(local_win, name, 16);
				if (strlen(name) == 0) {
					current_screen = PLAYERS_DIALOG;
...
void region_to_player() {
	...
	while(1) {
		...
		mvwprintw(local_win, 2, 2, "To give this region to a player, press Enter.\n");
		mvwprintw(local_win, 3, 2, "To unset region owner, press 'd'.\n");
		...
		while (current != NULL) {
			...
			if (counter >= section * 10 && counter < section * 10 + 10 && counter < nr_players) {
				...
				if (region->owner != NULL && region->owner->id == current->id) wprintw(local_win, " (current owner)\n");
				wattron(local_win, COLOR_PAIR(1));
			}
			current = current->next;
			counter++;
...
void rename_region_dialog() {
	...
	region_t *region = get_region_by_id(world->selected_region);
	mvwprintw(local_win, 2, 2, "To rename this region, type new name (press Enter to cancel):\n\n  ");
	char name[17] = {0};
	wgetnstr(local_win, name, 16);
	curs_set(FALSE);
	noecho();
...
void edit_player_dialog() {
	...
	mvwprintw(local_win, 5, 2, "Birthdate: %s of year %i", months[active_player->birthdate.tm_mon], active_player->birthdate.tm_year);
	mvwprintw(local_win, 6, 2, "Deathdate: %s of year %i", months[active_player->deathdate.tm_mon], active_player->deathdate.tm_year);
	mvwprintw(local_win, 7, 2, "Heir: %s", (active_player->heir == NULL ? "none": active_player->heir->name));
	mvwprintw(local_win, 8, 2, "Lord: %s", (active_player->lord == NULL ? "none": active_player->lord->name));
	...
	while (1) {
		...
		switch (user_move) {
			case '+':
				if (active_player->rank < KING) active_player->rank++;
				return;
				break;
			case '-':
				if (active_player->rank > KNIGHT) active_player->rank--;
				return;
				break;
			case 'b':
...
void rename_player_dialog() {
	...
	while(1) {
		wmove(local_win, 3, 0);
		wclrtoeol(local_win);
		mvwprintw(local_win, 2, 2, "Current name is '%s'. Type a new name, or press Enter to return.\n  New name: ", player->name);
		char name[17] = {0};
		wgetnstr(local_win, name, 16);
		if (strlen(name) == 0) {
			current_screen = EDIT_PLAYER_DIALOG;
		...
		mvwprintw(local_win, 23, 2, "Name not unique. Try another one.");
	}
}
...
void change_player_money_dialog() {
	...
	while(1) {
		wmove(local_win, 3, 0);
		wclrtoeol(local_win);
		mvwprintw(local_win, 2, 2, "Current amount is '%d'. Type new amount (0-999), or press Enter to return.\n  New amount: ", get_money(player));
		wgetnstr(local_win, money_ch, 3);
		if (strlen(money_ch) == 0) {
			current_screen = EDIT_PLAYER_DIALOG;
			return;
		...
		switch (stage) {
			case 1:
				if (error == 1) {
					...
					wmove(local_win, 3, 2);
					wclrtoeol(local_win);
					mvwprintw(local_win, 2, 2, "Error. Try another number (0-999):\n\n  Year: ");
				}
				else mvwprintw(local_win, 2, 2, "Current birthdate is '%s of %d'.\n  Type new year (0-999) and month (1-12), or press Enter to skip.\n  Year: ", months[player->birthdate.tm_mon], player->birthdate.tm_year);
				wgetnstr(local_win, birthyear_ch, 3);
				if (strlen(birthyear_ch) == 0) {
					mvwprintw(local_win, 4, 8, "%d", player->birthdate.tm_year);
					error = 0;
					stage = 2;
					break;
				}
			...
			case 2:
				if (error == 1) {
					...
					wmove(local_win, 9, 2);
					wclrtoeol(local_win);
					mvwprintw(local_win, 2, 2, "Error. Try another number (0-12):");
					mvwprintw(local_win, 4, 40, "Month: ");
				}
				else mvwprintw(local_win, 4, 40, "Month: ");
				wgetnstr(local_win, birthmon_ch, 2);
				if (strlen(birthmon_ch) == 0) {
					mvwprintw(local_win, 4, 47, "%s", months[player->birthdate.tm_mon]);
					error = 0;
					stage = 3;
					break;
				}
			...
			case 3:
				if (error == 1) {
					...
					wmove(local_win, 7, 2);
					wclrtoeol(local_win);
					mvwprintw(local_win, 8, 2, "Error. Try another number (0-999):\n\n  Year: ");
				}
				else mvwprintw(local_win, 6, 2, "Current deathdate is '%s of %d'.\n  Type new year (0-999) and month (1-12), or press Enter to skip.\n  Year: ", months[player->deathdate.tm_mon], player->deathdate.tm_year);
				uint16_t deathyear = 0;
				char deathyear_ch[3] = {0};
				wgetnstr(local_win, deathyear_ch, 3);
				if (strlen(deathyear_ch) == 0) {
					mvwprintw(local_win, 8, 8, "%d", player->deathdate.tm_year);
					error = 0;
					stage = 4;
					break;
				}
			...
			case 4:
				if (error == 1) {
					...
					wmove(local_win, 7, 2);
					wclrtoeol(local_win);
					mvwprintw(local_win, 6, 2, "Error. Try another number (0-12):");
					mvwprintw(local_win, 8, 40, "Month: ");
				}
				else mvwprintw(local_win, 8, 40, "Month: ");
				unsigned char deathmon = 0;
				char deathmon_ch[2] = {0};
				wgetnstr(local_win, deathmon_ch, 2);
				if (strlen(deathmon_ch) == 0) {
					mvwprintw(local_win, 8, 47, "%s", months[player->deathdate.tm_mon]);
					error = 0;
					stage = 5;
					break;
				}
			...
			case 5:
				curs_set(FALSE);
				noecho();
				mvwprintw(local_win, 23, 2, "Changes saved. Press any key to continue.");
				wgetch(local_win);
				current_screen = EDIT_PLAYER_DIALOG;
				return;
		}
...
void validate_dialog() {
	...
	for (i = 0; i < (80 - strlen(error_message)) / 2; i++) wprintw(local_win, " ");
	wprintw(local_win, error_message);
	free(error_message);

	msg = "Press any key to continue.";
	wmove(local_win, 12, 0);
	for (i = 0; i < (80 - strlen(msg)) / 2; i++) wprintw(local_win, " ");
	...
	while(1) {
		switch (stage) {
			case 0:
				...
				else mvwprintw(local_win, 2, 2, "Current date is '%s of %d'.\n  Type new year (0-999) and month (1-12), or press Enter to skip.\n  Year: ", months[world->current_time.tm_mon], world->current_time.tm_year);
				uint16_t year = 0;
				char year_ch[3] = {0};
				wgetnstr(local_win, year_ch, 3);
				if (strlen(year_ch) == 0) {
					mvwprintw(local_win, 4, 8, "%d", world->current_time.tm_year);
					error = 0;
					stage = 1;
					break;
				}
			...
			case 1:
				...
				if (strlen(mon_ch) == 0) {
					mvwprintw(local_win, 4, 47, "%s", months[world->current_time.tm_mon]);
					error = 0;
					stage = 2;
					break;
				}
				...
				break;
			case 2:
				mvwprintw(local_win, 22, 2, "Game time updated. Don't forget to validate game data. Press any key.");
				curs_set(FALSE);
				noecho();
				wgetch(local_win);
				current_screen = MAP_EDITOR;
...
void homage_dialog() {
	...
	while(1) {
		...
		else if (active_player->lord != NULL && active_player->lord == selected_player) {
			mvwprintw(local_win, 2, 2, "To unset lord, press 'd'");
			unset_lord_ok = 1;
		}
		else {
			mvwprintw(local_win, 2, 2, "To set lord, press Enter.\n");
			set_lord_ok = 1;
		}
		mvwprintw(local_win, 3, 2, "To scroll, press up/down keys.\n");
...
void diplomacy_dialog() {
	...
	while(1) {
		...
		switch (status) {
			case NEUTRAL:
			    mvwprintw(local_win, 2, 2, "To change to alliance, press 'a'");
				alliance_ok = 1;
			    mvwprintw(local_win, 3, 2, "To change to war, press 'w'");
				war_ok = 1;
				break;
			case ALLIANCE:
				mvwprintw(local_win, 2, 2, "To change to neutral, press 'n'");
				neutral_ok = 1;
				mvwprintw(local_win, 3, 2, "To change to war, press 'w'");
				war_ok = 1;
				break;
			case WAR:
				mvwprintw(local_win, 2, 2, "To change to alliance, press 'a'");
				alliance_ok = 1;
			    mvwprintw(local_win, 3, 2, "To change to neutral, press 'n'");
				neutral_ok = 1;
				break;
			case 3:
				break;
		...
		switch (user_move) {
			...
			case 1066:
				...
				break;
			case 'a': /* alliance */
				if (alliance_ok) set_diplomacy(active_player, selected_player, ALLIANCE);
				break;
			case 'n': /* neutral */
				if (neutral_ok) set_diplomacy(active_player, selected_player, NEUTRAL);
				break;
			case 'q': /* return */
				current_screen = EDIT_PLAYER_DIALOG;
				return;
				break;
			case 'w': /* declare war */
				if (war_ok) set_diplomacy(active_player, selected_player, WAR);
				break;
		}
	}
}
...
void help_dialog() {
	...
	mvwprintw(local_win, 8, 2, "'c' -- add tile to selected region or remove from its current region");
	...
	mvwprintw(local_win, 17, 2, "'w' -- toggle walkability of current tile");
