...
region_t *create_regionlist() {
	/* single instance */
	if (world->regionlist != NULL) return world->regionlist;
	world->regionlist = malloc(sizeof(region_t));
	if (world->regionlist == NULL) return NULL;
...
void change_tile_region(region_t *new_region, tile_t *tile) {
	/* same region */
	if (new_region != NULL && tile->region != NULL && tile->region->id == new_region->id) {
		return;
	}
	/* same NO region */
	...
	if (tile->region == NULL && new_region != NULL) {
		...
		new_region->size++;
		new_region->tiles = realloc(new_region->tiles, (new_region->size) * sizeof(tile_t *));
		new_region->tiles[new_region->size - 1] = tile;
		return;
	}
	/* reassign tile from one region to another */
	else if (tile->region != NULL && new_region != NULL) {
		tile_t **newlist = malloc(sizeof(tile_t *) * (tile->region->size - 1));
		j = 0;
		...
		tile->region->size--;
		new_region->tiles = realloc(new_region->tiles, (new_region->size + 1) * sizeof(tile_t *));
		new_region->size++;
		new_region->tiles[new_region->size - 1] = tile;
	...
	/* remove tile from region */
		...
		tile->region->size--;
		if (tile->region->tiles != NULL) free(tile->region->tiles);
		tile->region->tiles = newlist;
		tile->region = NULL;
		return;
	}
...
void change_region_owner(player_t *new_player, region_t *region) {
	/* same owner */
	if (region->owner != NULL && new_player != NULL && region->owner->id == new_player->id) {
		return;
	}
...
void change_region_name(char *new_name, region_t *region) {
	...
	region_t *current = world->regionlist;
	while (current != NULL) {
		if (strcmp(region->name, new_name) == 0) return; /* non-unique name */
		current = current->next;
	}
...
unsigned char claim_region(player_t *player, region_t *region) {
	...
	**/
	if (player == NULL || region == NULL) return 0;
	...
	if (current_owner == player) return 0;

	/* if current owner is in the region, stop here */
	piece_t *owners_noble = get_noble_by_owner(current_owner);
	region_t *present_location = world->grid->tiles[owners_noble->height][owners_noble->width]->region;
	if (region == present_location) return 0;
...
void clear_region(region_t *region) {
	if (region == NULL || region->size == 0) return;
	while (region->size > 0) change_tile_region(NULL, (region->tiles)[0]);
	free(region->tiles);
}

void remove_region(region_t *region) {
	if (world->regionlist == NULL) return;
	region_t *current = world->regionlist;
...
uint16_t count_tiles_by_owner(player_t *owner) {
	...
	region_t *current = world->regionlist;
	while (current != NULL) {
		if (current->owner != NULL && current->owner->id == owner->id) count += current->size;
		current = current->next;
	}
	return count;
}
...
uint16_t count_regions_by_owner(player_t *owner) {
	...
	region_t *current = world->regionlist;
	while (current != NULL) {
		if (current->owner != NULL && current->owner->id == owner->id) count ++;
		current = current->next;
	}
	return count;
}
...
void update_land_ranking() {
	...
	while (player != NULL) {
		...
		while (player2->id != player->id) {
			if (count_tiles_by_owner(player) <= count_tiles_by_owner(player2)) player->rank_land++;
			else player2->rank_land++;
...
grid_t *create_grid(const uint16_t height, const uint16_t width) {
	if (world->grid != NULL) return world->grid;
	int i, j;
...
void remove_grid() {
	...
	for (i = 0; i < world->grid->height; i++) {
		for (j = 0; j < world->grid->width; j++) {
			if (world->grid->tiles[i][j]->piece != NULL) remove_piece(world->grid->tiles[i][j]->piece);
...
unsigned int is_legal_move(const uint16_t src_height, const uint16_t src_width, const uint16_t dst_height, const uint16_t dst_width) {
	/* at least one move left */
	if (world->moves_left == 0) return 0;
	/* destination is within map limits */
	if (dst_height >= world->grid->height || dst_width >= world->grid->width) return 0;
	...
	if (diff_height * diff_height + diff_width * diff_width != 1) return 0;
	/* destination is walkable */
	if (world->grid->tiles[dst_height][dst_width]->walkable == 0) return 0;
	...
	if (src_piece == NULL || src_piece->owner->id != current_player->id) return 0;
	piece_t *dst_piece = world->grid->tiles[dst_height][dst_width]->piece;
	if (dst_piece != NULL) {
		/* no own piece on destination tile */
		if (dst_piece->owner->id == current_player->id) return 0;
		/* no allied piece on destination */
		if (get_diplomacy(current_player, dst_piece->owner) == 1) return 0;
	}
	return 1;
}

unsigned int move_piece(piece_t *piece, const uint16_t dst_height, const uint16_t dst_width) {
	uint16_t src_height = piece->height;
	uint16_t src_width = piece->width;
	unsigned int movable = is_legal_move(src_height, src_width, dst_height, dst_width);
...
void toggle_walkable(const uint16_t height, const uint16_t width) {
	...
	if (world->grid == NULL || world->grid->tiles[height][width]->piece != NULL) return;
	world->grid->tiles[height][width]->walkable = (world->grid->tiles[height][width]->walkable + 1) % 2;
}
